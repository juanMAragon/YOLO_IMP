...
architecture_config = [
    (7, 64, 2, 3),  # ConvLayer (kernel_size, output_filters, stride, padding)
    "M",  # Maxpool
    (3, 192, 1, 1),
    "M",
    (1, 128, 1, 0),
    ...
    (3, 512, 1, 1),
    "M",
    # [ConvLayer1, ConvLayer2, #repetitions]
...
class Yolov1(nn.Module):
    ...
    def _create_fcs(self, split_size, num_boxes, num_classes):
        ...
        return nn.Sequential(
            nn.Flatten(), 
            nn.Linear(1024*S*S, 496), #In the original paper the out_features should be 4096 
            nn.Dropout(0.0), 
            nn.LeakyReLU(0.1), 
            nn.Linear(496, S*S*(C+B*5)), # this will be reshaped to be (S,S,30) C+B*5=30
        )

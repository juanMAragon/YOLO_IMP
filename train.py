...
class Compose(object):
    ...
    def __call__(self, img, bboxes):
        for t in self.transforms:
            img, bboxes = t(img), bboxes # since we are only resizing t only applies to img, otherwise (data augmentation) should affect also bboxes (rotations, etc.)
        return img, bboxes
...
def main():
    ...
    train_dataset = VOCDataset(
        os.path.join('data', '8examples.csv'),
        transform = transform, 
        img_dir = IMG_DIR, 
        label_dir = LABEL_DIR
    )
